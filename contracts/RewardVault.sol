...
abstract contract RewardVault is Context, AccessControl, ERC20 {
    ...
    // Reward Percent in 100ths of a Percent. Default is 0.5%
    uint256 public _rewardPct = 50;
    ...
    function reclaim(address recipient, uint256 amount) public {
        ...
        // able to reclaim up to _reclaimMaxPct of Reward Vault funds on every 50th SaveTheChildren tweet
    ...
    function setRewardPct(uint256 rewardPct) public {
        require(hasRole(REWARD_ROLE, _msgSender()), "Only the registered account has access");
        _rewardPct = rewardPct;
    }
    ...
    function setTimelockAddress(address tokenAddr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only the registered account has access");
        _timelockAddress = tokenAddr;
    }

    function calculateRewardPct(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_rewardPct).div(10**4);
    }
